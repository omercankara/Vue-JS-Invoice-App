...
import  {computed,inject,watch} from "vue"
       ...
       const props  = defineProps({item:Object})  //invoiceItems tarafından gelen veriyi yakala
        const totalPrice = computed(()=>props.item.qty * props.item.unit_price)  //birim fiyat ile adeti çarp
        
        watch(totalPrice, (totalPrice) => {
                props.item.total_price = totalPrice //propsun içindeki totalPriceı yukarıdaki veri ile eşitle
        })
              const DeleteInvoiceItem = inject("DeleteInvoiceItem") //invoiceContent componentinden provide ile  fonksiyonu yakala
</script>